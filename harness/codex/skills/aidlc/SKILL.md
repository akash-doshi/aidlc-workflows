...
After any stage transition (a `report` that advances state), if the `aidlc-dashboard` MCP server is configured, call its `aidlc_dashboard` tool once to refresh the live panel. It is read-only and never blocks the loop; skip silently if the tool is unavailable.
```
...
- **`gate: true`** — after the stage body produces its artifacts:
  1. **Reviewer step (§12a):** If `directive.reviewer` is present, run the reviewer as a separate pass framed by the reviewer persona (`.codex/agents/<directive.reviewer>.toml`). Pass ONLY: the stage definition path, the Q&A file path, and the produced artifact paths — NOT `memory.md` or `plan.md` (the reviewer forms independent judgement). The reviewer appends a `## Review` section with a READY / NOT-READY verdict to the primary artifact. Read the verdict: if NOT-READY and iterations < `directive.reviewer_max_iterations` (default 2), re-run the stage body to address the findings, then re-review; if READY or iterations exhausted, proceed. See `aidlc-common/protocols/stage-protocol.md` §12a.
  2. Run stage-completion verification (artifacts exist, guardrails respected).
  3. Unless in test-run mode, run the **§13 learnings ritual**: `bun .codex/tools/aidlc-learnings.ts surface --slug <slug>`, render the structured question + free-text channel (per `question-rendering.md`), run the admission conflict-check against `aidlc-rules/aidlc-org.md`, then `bun .codex/tools/aidlc-learnings.ts persist --slug <slug> --selections-json <path>`. Advisory and additive — it never blocks the gate. See `aidlc-common/protocols/stage-protocol.md` §13.
